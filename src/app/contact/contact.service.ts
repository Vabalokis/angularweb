...
export class AppService {
  ...
  sendEmail(message: IMessage): Observable<IMessage> | any {
    const nameval = (<HTMLInputElement>document.getElementById("nametextbox"))
      .value;
    const emailval = (<HTMLInputElement>document.getElementById("emailtextbox"))
      .value;
    const messageval = (<HTMLInputElement>(
      document.getElementById("messagetextbox")
    )).value;

    if (nameval !== "" && emailval !== "" && messageval !== "") {
      this.clearForm();
      return this.http
        .post(this.emailUrl, message)
        .map(response => {
